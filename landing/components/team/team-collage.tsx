...
function CollageLabel({
  ...
  return (
    <div
      className={`absolute z-20 border border-[#111111] px-3 py-1 text-[11px] font-semibold text-[#111111] ${className}`}
      aria-hidden="true"
    >
      {children}
    </div>
...
function CollageFrame({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative border border-[#111111] bg-[#f7f6f2] p-3 shadow-[6px_6px_0_0_#ff5f36] md:p-4">
      {children}
    </div>
  );
}
...
export function TeamHeroPhoto({
  ...
  return (
    <CollageFrame>
      <CollageLabel className="-left-2 -top-3 -rotate-6 bg-[#111111] text-white">
  ...
  return (
    <CollageFrame>
      ...
      <TeamPortrait
        ...
        name={member.name}
        priority
        className="relative aspect-[4/5] overflow-hidden border border-[#111111] bg-[#ebe9e3]"
    ...
    <CollageFrame>
      ...
      <div className="grid grid-cols-2 gap-2">
        <TeamPortrait
          ...
          name={large.name}
          priority
          className="relative col-span-1 row-span-2 aspect-[3/4] overflow-hidden border border-[#111111] bg-[#ebe9e3]"
        />
        <TeamPortrait
          photoUrl={small.photoUrl}
          alt={small.photoAltText ?? `Portrait of ${small.name}`}
          name={small.name}
          className="relative aspect-square overflow-hidden border border-[#111111] bg-[#ebe9e3]"
          sizes="25vw"
        />
        <div
          className="flex aspect-square items-center justify-center border border-dashed border-[#111111] bg-[#efede7] text-center font-display text-[10px] font-bold uppercase tracking-[0.16em] text-[#737373]"
          aria-hidden="true"
        >
          NextGen CTO
        </div>
...
function FullCollage({
  ...
  return (
    <CollageFrame>
      ...
      <CollageLabel className="-right-3 bottom-10 rotate-3 bg-[#f7f6f2] max-md:hidden">
        <span className="mr-2 inline-block size-2 rounded-full bg-[#ff5f36] align-middle" />
        Chai status: critical
      </CollageLabel>
      <CollageLabel className="-left-3 top-1/2 -rotate-12 bg-[#f7f6f2] max-md:hidden">
        Probably debugging
      </CollageLabel>
      <div className="grid grid-cols-2 grid-rows-2 gap-2 border border-[#111111] bg-[#e2e2e2] p-2">
        <TeamPortrait
...
export function TeamCollageFallback() {
  ...
  return (
    <CollageFrame>
      ...
      <div className="border border-[#111111] bg-[#efede7] p-5 md:p-8">
        <p className="font-display text-2xl font-bold tracking-tight text-[#111111] md:text-3xl">
          NEXTGEN CTO
        </p>
        <p className="mt-1 font-display text-[10px] font-bold uppercase tracking-[0.2em] text-[#737373]">
          Team photo pending
        </p>
        <div className="mt-6 grid grid-cols-4 gap-2" aria-hidden="true">
          {placeholders.map((label, index) => (
            <div
              key={label + index}
              className="flex aspect-square items-center justify-center border border-[#111111] bg-[#e6e4de] font-display text-sm font-bold text-[#737373]"
            >
              {label}
            </div>
          ))}
        ...
        <p
          className="mt-6 inline-block -rotate-1 border border-[#111111] bg-[#f7f6f2] px-3 py-1 text-[11px] font-semibold italic text-[#ff5f36]"
          aria-hidden="true"
        >
