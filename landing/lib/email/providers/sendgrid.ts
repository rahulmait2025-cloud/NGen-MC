...
function pickMessageId(raw: unknown): string | undefined {
  ...
  const value = headers['x-message-id'] ?? headers['X-Message-Id'] ?? headers['x-message-id'.toLowerCase()];
    ...
    try {
      const attachments = input.attachments?.map((attachment) => {
        ...
        if (attachment.contentId) {
          nextAttachment.content_id = attachment.contentId;
        }
      ...
      return {
        ...
        errorCode:
          ...
          (typeof maybeError.response?.statusCode === 'number'
            ? String(maybeError.response.statusCode)
            : undefined) ??
          'sendgrid_error',
