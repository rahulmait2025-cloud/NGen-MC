...
/**
 * LMS Razorpay Webhook Receiver.
 *
 * LMS-ONLY endpoint. Handles webhook events for LMS-originated orders.
 * Register in Razorpay Dashboard: https://<lms-domain>/api/lms/webhooks/razorpay
 *
 * Scope: Updates orders where source = 'lms'. Does NOT process CollegeAdmin orders.
 */
  ...
  try {
    ...
    try {
      if (eventType === 'payment.captured') {
        ...
        if (orderId) {
          const { data: order } = await admin.from('orders').select('status').eq('id', orderId).eq('source', 'lms').single();
          if (order && (order as { status: string }).status === 'paid') {
            status = 'duplicate';
          } else if (orderId) {
            await admin.from('orders').update({ status: 'paid', paid_at: new Date().toISOString() }).eq('id', orderId).eq('source', 'lms');
        ...
        if (orderId) {
          await admin.from('orders').update({ status: 'failed', metadata: { webhook_failed: true } }).eq('id', orderId).eq('source', 'lms');
        }
      } else if (eventType === 'refund.processed') {
        const paymentId = (payload.entity as Record<string, unknown>)?.id as string;
        if (paymentId) {
          await admin.from('orders').update({ status: 'refunded', refunded_at: new Date().toISOString() }).eq('gateway_payment_id', paymentId).eq('source', 'lms');
        }
      }
    } catch {
      status = 'failed';
