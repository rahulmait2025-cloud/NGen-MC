...
import { validateLmsCoupon } from '@/lib/lms/commerce/services/coupons';
import type { LmsEntityType } from '@/lib/lms/commerce/services/coupons';
import { z } from 'zod';
...
/**
 * POST /api/lms/coupons/validate
 *
 * Validate an LMS coupon server-side and return the calculated discount.
 *
 * Request body:
 * {
 *   "code": "SAVE20",
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await validateLmsCoupon({
      code,
    ...
    console.error('[api/lms/coupons/validate] LMS coupon validation failed:', error);
    return NextResponse.json(
      {
        valid: false,
        error: 'Failed to validate LMS coupon',
