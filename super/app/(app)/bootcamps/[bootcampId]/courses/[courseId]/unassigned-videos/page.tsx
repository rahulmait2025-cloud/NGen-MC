...
export default async function BootcampUnassignedVideosPage({
  params,
}: {
  ...
  // Guardrails: must be a bootcamp course under this bootcamp.
  ...
  const videoImpacts = new Map(
    await Promise.all(
      videos.map(
        async (video) => [video.id, await getVideoDeleteImpact(video.id)] as const,
      ),
    ),
  );
      ...
      <div className="flex items-center gap-4">
        <div>
          ...
          <div className="text-sm text-muted-foreground mt-1 flex items-center gap-2">
            <span className="font-medium text-primary">{bootcamp.title}</span>
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  These videos are uploaded to the course folder but are not assigned
                  to any module. They will not be visible to students until assigned.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            {videos.length === 0 ? (
              <div className="text-center py-20 bg-muted/20 rounded-xl border border-dashed">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  All videos in this course are currently assigned to modules.
                </p>
                <Button variant="outline" className="mt-6" asChild>
                  <Link href={`/bootcamps/${bootcampId}/courses/${courseId}`}>
                    Back to Course
                  </Link>
                </Button>
              </div>
  ...
  switch (status) {
    ...
    case 'queued':
      return (
        <Badge
          variant="outline"
          className="text-amber-600 border-amber-200 bg-amber-50 animate-pulse"
        >
          Processing
        </Badge>
      );
