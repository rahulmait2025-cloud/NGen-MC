-- Migration: refine_master_course_pillar_stats_counts
-- 1. Updates the stats view to correctly count only non-deleted courses and modules.
-- 2. Filters out 'deleted' courses/modules (unpublished + no TP folder) from the aggregates.
...
comment on view public.master_course_pillar_stats is
  'Aggregated stats for Master Course Pillars. Filters out fully deleted pillars, courses, and modules from counts.';
