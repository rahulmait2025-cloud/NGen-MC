-- Migration: 00248_database_performance_optimization.sql
-- Description: Consolidated DB-side RPC function and composite index optimizations.

BEGIN;

-- ─── 1. Composite & Performance Indexes ──────────────────────────────────────

-- Index for colleges lower(slug) lookup to avoid sequential scans on auth context checks
CREATE INDEX IF NOT EXISTS idx_colleges_lower_slug ON public.colleges (lower(slug));

-- Index for student_progress batch completion queries
CREATE INDEX IF NOT EXISTS idx_student_progress_student_completed_item 
  ON public.student_progress (student_id, completed, item_id);

-- Index for student_video_progress batch completion queries
CREATE INDEX IF NOT EXISTS idx_student_video_progress_student_completed_lesson 
  ON public.student_video_progress (student_id, completed, lesson_id);

-- Index for course_variant_items resolution
CREATE INDEX IF NOT EXISTS idx_course_variant_items_variant_item 
  ON public.course_variant_items (course_variant_id, master_course_item_id);

-- Index for bundle_items resolution
CREATE INDEX IF NOT EXISTS idx_bundle_items_bundle_reference 
  ON public.bundle_items (bundle_id, reference_id);

-- pg_cron performance indexes & history cleanup job configuration
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    BEGIN
      EXECUTE 'CREATE INDEX IF NOT EXISTS idx_cron_job_run_details_jobid ON cron.job_run_details (jobid)';
      EXECUTE 'CREATE INDEX IF NOT EXISTS idx_cron_job_run_details_start_time ON cron.job_run_details (start_time)';
    EXCEPTION WHEN insufficient_privilege THEN
      RAISE NOTICE 'Skipping cron index creation due to insufficient privileges.';
    WHEN OTHERS THEN
      RAISE NOTICE 'Skipping cron index creation due to error.';
    END;
    
    BEGIN
      PERFORM cron.unschedule('cleanup_cron_history') FROM cron.job WHERE jobname = 'cleanup_cron_history';
      PERFORM cron.schedule('cleanup_cron_history', '0 0 * * *', 'DELETE FROM cron.job_run_details WHERE start_time < NOW() - INTERVAL ''7 days''');
    EXCEPTION WHEN insufficient_privilege THEN
      RAISE NOTICE 'Skipping cron scheduling due to insufficient privileges.';
    WHEN OTHERS THEN
      RAISE NOTICE 'Skipping cron scheduling due to error.';
    END;
  END IF;
END $$;


-- ─── 2. Unified Entitled Courses RPC Function ────────────────────────────────
...
BEGIN
  ...
  SELECT coalesce(array_agg(id), '{}') INTO v_published_pillars
  FROM public.master_course_pillars
  WHERE publish_status = 'published';

  SELECT coalesce(array_agg(id), '{}') INTO v_published_bootcamps
  ...
  IF p_exclude_bundle_only THEN
    ...
    SELECT coalesce(array_agg(id), '{}') INTO v_final_course_ids
    FROM unnest(v_final_course_ids) AS id
    WHERE NOT (id = ANY(v_bundle_ids) AND NOT (id = ANY(v_direct_ids)));
  END IF;
  ...
  completed_others AS (
    ...
    WHERE sp.student_id = p_student_id
      ...
      AND sp.item_id IN (SELECT ci.id FROM course_items ci WHERE ci.item_type <> 'video')
  ),
  completed_all AS (
    SELECT item_id FROM completed_videos
    UNION
    SELECT item_id FROM completed_others
  ),
  progress_stats AS (
    SELECT
      ci.master_course_id,
      count(ci.id) as total_items,
      count(ca.item_id) as completed_items
    FROM course_items ci
    LEFT JOIN completed_all ca ON ca.item_id = ci.id
...
-- ─── 3. Function Permissions & RLS Hardening ────────────────────────────────
