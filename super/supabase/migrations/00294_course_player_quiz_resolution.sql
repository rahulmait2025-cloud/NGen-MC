-- Migration: 00294_course_player_quiz_resolution.sql
-- Description: Retrieve assessment_id for master_course_items inside get_student_course_player_payload.
...
BEGIN
  ...
  modules_with_items AS (
    ...
    GROUP BY md.id, md.master_course_id, md.title, md.description, md.slug, md.sort_order, md.publish_status, md.metadata, md.visible_to_students, md.created_at
    ORDER BY md.sort_order ASC, md.created_at ASC
  )
  ...
  IF p_item_id IS NOT NULL THEN
    ...
    IF v_resolved_variant_id IS NULL OR EXISTS (SELECT 1 FROM public.course_variant_items WHERE course_variant_id = v_resolved_variant_id AND master_course_item_id = p_item_id) THEN
      ...
      FROM (
        SELECT 
          id, master_course_id, module_id, title, slug, description, item_type, sort_order, publish_status, video_asset_id, is_preview AS preview_enabled, metadata, resource_id, assessment_id
