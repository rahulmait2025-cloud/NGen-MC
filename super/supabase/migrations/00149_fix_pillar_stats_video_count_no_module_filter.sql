-- Migration: 00149_fix_pillar_stats_video_count_no_module_filter.sql
-- Purpose: Fix video_count in pillar stats by removing module_id filter from items join.
--          Migration 00148 incorrectly filtered items by mci.module_id = mcm.id,
--          which excluded items from unpublished modules from the count.
--          This migration counts all published video items directly from courses.
...
select
  ...
  -- Count all published video items directly from courses (no module filter)
...
comment on view public.master_course_pillar_stats is
  'Aggregated stats for Master Course Pillars. video_count counts all published video items from courses, matching actual course content.';
