-- Migration: 00258_lms_analytics_and_course_fetching_performance.sql
-- Description: Optimize get_student_entitled_courses RPC by replacing scalar subqueries with a joined CTE, reducing N+1 query overhead.
...
BEGIN
  ...
  completed_videos AS (
    ...
    JOIN course_items ci ON ci.item_id = svp.lesson_id
    WHERE svp.student_id = p_student_id
      AND svp.completed = true
      AND ci.item_type = 'video'
  ),
  completed_others AS (
    SELECT sp.item_id AS cmp_item_id
    FROM public.student_progress sp
    JOIN course_items ci ON ci.item_id = sp.item_id
  ...
  course_bundles_info AS (
    SELECT
      ...
      true AS has_full_bundle_access
  ...
  -- Optimize: student entitlements info resolved once via join CTE instead of N+1 scalar subqueries
  student_entitlements_info AS (
    SELECT DISTINCT ON (se.master_course_id) se.master_course_id, se.source_type
    FROM public.student_entitlements se
    WHERE se.student_id = p_student_id
      AND se.status = 'active'
      AND (se.valid_from IS NULL OR se.valid_from <= now())
      AND (se.valid_until IS NULL OR se.valid_until >= now())
    ORDER BY se.master_course_id, (se.source_type = 'b2c_direct') DESC
  )
  SELECT
    mc.id,
    mc.created_at,
    ...
    CASE
      ...
      ELSE
        coalesce(
          CASE
            WHEN sei.source_type = 'b2c_direct' THEN 'Purchased Course'
            WHEN sei.source_type IN ('college_assignment', 'b2b_college') THEN 'College Assigned'
            ...
            WHEN sei.master_course_id IS NOT NULL THEN 'Free Enrollment'
            ELSE NULL
          END,
    ...
    CASE
      ...
      ELSE
        ...
        ARRAY(
          SELECT lbl
          FROM unnest(ARRAY[
            CASE WHEN sei.source_type = 'b2c_direct' THEN 'Purchased Course' ELSE NULL END,
            CASE WHEN sei.source_type IN ('college_assignment', 'b2b_college') THEN 'College Assigned' ELSE NULL END,
            ...
            CASE WHEN sei.master_course_id IS NOT NULL AND sei.source_type NOT IN ('b2c_direct', 'college_assignment', 'b2b_college') THEN 'Free Enrollment' ELSE NULL END
          ]) AS lbl
  ...
  LEFT JOIN student_entitlements_info sei ON sei.master_course_id = mc.id
