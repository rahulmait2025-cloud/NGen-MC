-- Migration: 00262_fix_analytics_include_all_lessons.sql
-- Description: Fix get_student_analytics_payload RPC to count ALL watched lessons,
-- not just those in currently accessible courses.

BEGIN;

-- Drop and recreate with ONLY the totalLecturesWatched line changed
-- (section F line 116 removed the AND lesson_id = ANY(v_valid_item_ids) filter)
-- and section J completedLectures removed the same filter.
-- All other sections remain IDENTICAL to the original 00254 function.
...
BEGIN
  ...
  -- E. Calculate average completion percentage
  ...
  v_overview := jsonb_build_object(
    ...
    'totalLecturesWatched', (SELECT count(1) FROM public.student_video_progress WHERE student_id = p_student_id AND unique_watched_seconds > 0),
    ...
    SELECT
      ...
      (
        ...
        FROM (
          ...
          WHERE student_id = p_student_id AND last_watched_at >= d.date_val::date AND last_watched_at < (d.date_val::date + interval '1 day')
