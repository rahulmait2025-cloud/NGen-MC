-- Migration: 00249_course_player_payload_optimization.sql
-- Description: Unified course player payload RPC for student LMS.

BEGIN;

-- ─── 1. Targeted Performance Indexes for Player Engagement Tables ───────────

CREATE INDEX IF NOT EXISTS idx_student_progress_student_item_lookup
  ON public.student_progress (student_id, item_id);

CREATE INDEX IF NOT EXISTS idx_student_video_progress_student_lesson_lookup
  ON public.student_video_progress (student_id, lesson_id);

CREATE INDEX IF NOT EXISTS idx_lesson_resources_course_item
  ON public.lesson_resources (master_course_id, item_id);

CREATE INDEX IF NOT EXISTS idx_student_lesson_bookmarks_student_item
  ON public.student_lesson_bookmarks (student_id, item_id);

CREATE INDEX IF NOT EXISTS idx_course_resources_player_attachment
  ON public.course_resources (master_course_id, parent_item_id, resource_scope, publish_status, visible_to_students);


-- ─── 2. Course Player Payload RPC Function ──────────────────────────────────
...
BEGIN
  ...
  IF NOT v_allowed THEN
    ...
    IF NOT v_allowed AND p_variant_id IS NOT NULL THEN
      SELECT EXISTS (
        SELECT 1 FROM public.student_content_entitlements
        WHERE student_id = p_student_id AND assigned_entity_type = 'variant'
          AND assigned_entity_id::uuid = p_variant_id AND status = 'active'
          AND (valid_until IS NULL OR valid_until >= now())
      ) INTO v_allowed;
    END IF;
  ...
  -- 5. Fetch Modules and Items with Student Progress
  WITH progress_entries AS (
    SELECT 
      ...
      jsonb_build_object(
        ...
        'item_id', sp.item_id,
        'entitlement_id', null,
        'watched_seconds', sp.watched_seconds,
        'total_seconds', sp.total_seconds,
        'last_position_seconds', sp.last_position_seconds,
    ...
    SELECT 
      ...
      jsonb_build_object(
        'id', '',
        'student_id', p_student_id,
        'item_id', svp.lesson_id,
        'entitlement_id', null,
        'watched_seconds', svp.unique_watched_seconds,
        'total_seconds', svp.video_duration_seconds,
        'last_position_seconds', svp.last_position_seconds,
        'completed', svp.completed,
        'completed_at', null
      ) as prog_obj
  ...
  modules_with_items AS (
    ...
    LEFT JOIN items_data id ON id.module_id = md.id
