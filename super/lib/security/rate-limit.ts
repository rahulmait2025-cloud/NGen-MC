...
function _addRateLimitHeaders(
  response: NextResponse,
  result: RateLimitResult,
  limit: number
): NextResponse {
  const rateLimitHeaders = getRateLimitHeaders(result, limit);
...
function _withRateLimitAndSecurity(
  request: NextRequest,
  options: RateLimitOptions
): Promise<{ limited: boolean; result: RateLimitResult; ip: string }> {
  const ip = getRequestIp(request);
  return checkRateLimit({ ...options, key: `${options.key}:${ip}` });
}

async function checkRateLimit(options: RateLimitOptions): Promise<{
  limited: boolean;
  result: RateLimitResult;
  ip: string;
}> {
