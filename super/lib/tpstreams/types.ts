...
export interface TpUsageParams {
  month?: number;
  year?: number;
  day?: number;
...
export interface TpUsageRecord {
  readonly date: string;
  readonly bandwidth_used_bytes: number;
  readonly active_storage_bytes: number;
  readonly deleted_storage_bytes: number;
  readonly total_storage_bytes: number;
  readonly subtitle_generation_cost: number;
  readonly live_stream_usage_seconds?: number;
}

// ─── Uploader SDK ─────────────────────────────────────────────────────────────

export interface TpUploaderEventData {
  readonly name?: string;
  readonly asset_id?: string;
  readonly progress_percentage?: number;
  readonly status?: string;
  readonly error?: string | { readonly message?: string; readonly detail?: string; readonly error?: string };
}

export interface TpStreamsUploaderInstance {
  selectFiles: (files: File[]) => void;
  upload: (folderId?: string | null) => void;
  on: (
    event: 'uploadProgress' | 'uploadSuccess' | 'uploadError',
    callback: (data: TpUploaderEventData) => void,
  ) => void;
}
