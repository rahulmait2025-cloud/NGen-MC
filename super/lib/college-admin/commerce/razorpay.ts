...
/**
 * CollegeAdmin Razorpay Payment Provider Implementation.
 *
...
import {
  ...
  CollegeGatewayOrderRequest,
  CollegeGatewayOrderResponse,
  CollegePaymentVerificationRequest,
  CollegePaymentVerificationResult,
  CollegeWebhookVerificationRequest,
  CollegeWebhookVerificationResult,
  CollegeRefundRequest,
  CollegeRefundResponse,
} from './provider';
...
function getConfig() {
  ...
  if (!keyId) {
    throw new CollegeRazorpayConfigError('RAZORPAY_KEY_ID is not set in environment variables.');
  }
  if (!keySecret) {
    throw new CollegeRazorpayConfigError('RAZORPAY_KEY_SECRET is not set in environment variables.');
  }

  return { keyId, keySecret, webhookSecret };
}

class CollegeRazorpayConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'CollegeRazorpayConfigError';
  }
}

class CollegeRazorpayApiError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = 'CollegeRazorpayApiError';
  }
}

// ─── HTTP Client ──────────────────────────────────────────────────────────────────

async function collegeRazorpayFetch<T>(
  path: string,
  options: {
  ...
  if (!response.ok) {
    ...
    throw new CollegeRazorpayApiError(
...
// ─── CollegeRazorpay Provider Implementation ──────────────────────────────────────

class CollegeRazorpayProvider implements CollegePaymentProvider {
   ...
   * POST /v1/orders
   */
  async createOrder(request: CollegeGatewayOrderRequest): Promise<CollegeGatewayOrderResponse> {
    const response = await collegeRazorpayFetch<{
      id: string;
      amount: number;
      currency: string;
      status: string;
   ...
   */
  async verifyPayment(request: CollegePaymentVerificationRequest): Promise<CollegePaymentVerificationResult> {
   ...
   */
  async verifyWebhook(request: CollegeWebhookVerificationRequest): Promise<CollegeWebhookVerificationResult> {
    const { webhookSecret } = getConfig();
    const { body, signature } = request;

    if (!webhookSecret) {
      throw new CollegeRazorpayConfigError(
   ...
   */
  async refund(request: CollegeRefundRequest): Promise<CollegeRefundResponse> {
  ...
  async fetchPayment(gatewayPaymentId: string): Promise<Record<string, unknown>> {
    return collegeRazorpayFetch(`/payments/${gatewayPaymentId}`);
  }
  ...
  async fetchOrder(gatewayOrderId: string): Promise<Record<string, unknown>> {
    return collegeRazorpayFetch(`/orders/${gatewayOrderId}`);
  }
}
...
/**
 * Get the CollegeAdmin Razorpay provider instance.
 * Use this throughout the CollegeAdmin commerce system.
 */
export function getCollegeRazorpayProvider(): CollegeRazorpayProvider {
  return new CollegeRazorpayProvider();
}

/**
 * Get the public Razorpay key (safe for browser exposure).
 */
export function getCollegeRazorpayPublicKey(): string {
  const keyId = process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID ?? process.env.RAZORPAY_KEY_ID;
  if (!keyId) {
    throw new CollegeRazorpayConfigError('NEXT_PUBLIC_RAZORPAY_KEY_ID is not set.');
  }
  return keyId;
}
