...
/**
 * CollegeAdmin Coupon Service.
 *
 * Server-side coupon validation and management for the CollegeAdmin portal.
 * All discount calculations happen here — never trust client-side amounts.
 *
 * FULLY ISOLATED — no imports from shared lib/services/coupons.ts.
...
export interface CollegeCouponValidationResult {
  valid: boolean;
  coupon?: CollegeCouponRecord;
  discountAmountMinor: number;
  errorMessage?: string;
}

export interface CollegeCouponRecord {
  id: string;
  code: string;
  ...
  applicable_sources: Array<'lms' | 'college_admin'>;
}

export interface CollegeValidateCouponInput {
  code: string;
...
/**
 * Validate a CollegeAdmin coupon against an order.
 *
 ...
 * - Source is restricted to "college_admin" only
 *
 * Returns the calculated discount amount.
 */
export async function validateCollegeCoupon(
  input: CollegeValidateCouponInput,
): Promise<CollegeCouponValidationResult> {
  ...
  const record = coupon as CollegeCouponRecord;
  ...
  if (record.min_order_amount_minor && input.orderAmountMinor < record.min_order_amount_minor) {
    return {
      valid: false,
      discountAmountMinor: 0,
      errorMessage: `Minimum order amount is Rs${(record.min_order_amount_minor / 100).toFixed(2)}`,
    };
  }

  // Check source applicability — CollegeAdmin ONLY
  if (!record.applicable_sources.includes('college_admin')) {
    return {
      valid: false,
      discountAmountMinor: 0,
      errorMessage: 'Coupon is not valid for the CollegeAdmin portal',
    };
  }
...
/**
 * Record a CollegeAdmin coupon usage after a successful order.
 * Called as part of the order completion flow.
 */
export async function recordCollegeCouponUsage(params: {
  couponId: string;
  orderId: string;
  purchaserUserId?: string;
  purchaserEmail: string;
  ...
  if (error) {
    console.error('[college-admin/coupons] Failed to record coupon usage:', error);
