...
function FounderAnnotation({ children, className }: { children: string; className: string }) {
  return (
    <div
      className={`absolute z-10 border border-[#111111] bg-[#f7f6f2] px-3 py-1 text-[11px] font-semibold italic text-[#111111] dark:border-[#2a2d32] dark:bg-[#1a1d22] dark:text-[#c5c2bc] ${className}`}
      aria-hidden="true"
    >
      {children}
    </div>
...
export function FounderFeature({ founders }: { founders: PublicTeamMember[] }) {
  ...
  return (
    <section className="border-b border-[#111111] bg-white dark:border-[#2a2d32] dark:bg-[#14161a]">
      {founders.map((founder, index) => {
        ...
        return (
          <article
            key={founder.id}
            className="relative border-b border-[#111111] px-5 py-16 last:border-b-0 md:px-16 md:py-20 dark:border-[#2a2d32]"
          >
            <p
            ...
            <div className="mx-auto grid max-w-7xl grid-cols-1 items-start gap-10 md:grid-cols-12 md:gap-12">
              <div className={`md:col-span-5 ${reversed ? 'md:order-2' : ''}`}>
                <div className="relative">
                  ...
                  <TeamPortrait
                    ...
                    name={founder.name}
                    priority={isPrimary}
                    className={`relative overflow-hidden border-2 border-[#111111] bg-[#ebe9e3] dark:border-[#3a3d42] dark:bg-[#1e2024] ${
              ...
              <div className={`md:col-span-7 md:pt-10 ${reversed ? 'md:order-1' : ''}`}>
                {isPrimary ? (
                  <h2 className="font-display text-2xl font-bold leading-tight tracking-tight text-[#111111] md:text-4xl dark:text-[#e8e5df]">
                ...
                <div className="mt-8 border-t border-[#111111] pt-6 dark:border-[#2a2d32]">
                  <p className="font-display text-xs font-bold uppercase tracking-[0.16em] text-[#ff5f36]">
                    {founder.shortRole ?? 'Founder'}
                  </p>
                  <h3 className="mt-2 font-display text-2xl font-bold text-[#111111] md:text-3xl dark:text-[#e8e5df]">
                    {founder.name}
                  </h3>
                  ...
                  {founder.location ? (
                    <p className="mt-2 text-sm text-[#666666] dark:text-[#8a8780]">{founder.location}</p>
                  ) : null}
                </div>

                {bio ? (
                  <p className="mt-6 max-w-2xl whitespace-pre-line text-base leading-relaxed text-[#555555] md:text-lg dark:text-[#9a9790]">
                    {bio}
                  </p>
                ) : null}
                ...
                {isPrimary ? (
                  <blockquote className="my-6 border-l-4 border-[#ff5f36] py-1 pl-5">
                    <p className="font-display text-lg italic text-[#111111] md:text-xl dark:text-[#e8e5df]">
