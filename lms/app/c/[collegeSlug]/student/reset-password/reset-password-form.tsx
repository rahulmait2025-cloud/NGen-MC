...
import { updateStudentRecoveryPassword } from './actions';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';

interface StudentResetPasswordFormProps {
  collegeSlug: string;
...
export function StudentResetPasswordForm({
  collegeSlug,
  hasRecoverySession,
}: StudentResetPasswordFormProps): ReactNode {
  ...
  const forgotHref = `/c/${encodeURIComponent(collegeSlug)}/student/forgot-password`;
  ...
  useEffect(() => {
    ...
    async function establishRecoverySession() {
      ...
      console.warn('[student-reset-password] client fallback', {
      ...
      if (hash.includes('access_token') && hash.includes('refresh_token')) {
        ...
        if (access_token && refresh_token) {
          ...
          if (!sessionError) {
            ...
            if (!cancelled) {
              setState({ ready: true, noSession: !user });
            }
            return;
          }
        }
      ...
      console.warn('[student-reset-password] session check', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateStudentRecoveryPassword(state.password);
      if (!result.ok) {
        setState({ error: result.message, loading: false });
        return;
      }

      push(`${loginHref}?reset=success`);
    } catch {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
        <div className="w-full max-w-sm space-y-6 rounded-2xl border bg-card p-8 shadow-lg">
          ...
          <Button variant="outline" className="w-full h-10 rounded-xl" onClick={() => push(forgotHref)}>
            Request new link
          </Button>
