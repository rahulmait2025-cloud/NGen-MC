...
import { getJobDetail, getStudentApplication, canStudentApplyToJob } from '@/lib/services/student-jobs';
import { JobApplyForm } from '@/components/jobs/job-apply-form';
import { Building2, ArrowLeft } from 'lucide-react';
import Link from 'next/link';

export default async function StudentJobApplyPage({
  params,
}: {
  ...
  const [existingApplication, eligibility] = await Promise.all([
  ...
  if (existingApplication && existingApplication.status !== 'withdrawn') {
    redirect(`/c/${collegeSlug}/student/jobs/${jobId}/edit`);
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-16">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-[28px] sm:text-[32px] font-semibold tracking-tight">Apply for Position</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Submit your application for <span className="font-medium text-foreground">{job.title}</span> at <span className="font-medium text-foreground">{job.company_name}</span>.
          </p>
        </div>
      </div>
